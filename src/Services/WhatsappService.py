...
async def get_mensagens(session: AsyncSession, motorista_id: Optional[UUID] = None) -> List[MensagemChat]:
    """
    Retorna o histórico de conversas do WhatsApp.
    Opcionalmente filtra por um motorista específico.
    """
    query = select(MensagemChat)
    if motorista_id:
...
async def send_mensagem(session: AsyncSession, data: MensagemCreate) -> List[MensagemChat]:
    """
    ...
    **RodovIA** ('SISTEMA_IA') responderá de forma automatizada simulando inteligência artificial.
    ...
    await PessoasService.get_motorista_joined(session, data.motorista_id)
